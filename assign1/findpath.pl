...
edge(e, b, 1).
edge(e, d, 1).
edge(e, j, 2). % Added my own extra vertex for testing.
edge(d, e, 1).
edge(d, b, 3).
edge(d, c, 1).
edge(c, d, 1).
...
findpath(X, Y, W, Path, Visited) :-
                 ...
                 edge(Z, Y, W1),                 % Find some Z that definitely has an edge with Y.
                 \+ member(Z, Visited),          % Z Must not have been visited yet, lest that be a cycle.
                 append(Visited, [Y], V),        % Mark Y as having been visited. Do not mark Z yet as we
                                                 %    have not found that path yet.
                 findpath(X, Z, W2, Path1, V), % Find a path from X to Z.
